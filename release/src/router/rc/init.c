...
/* glibc >= 2.8 uses per-thread malloc arenas which may increase memory usage.
 * Limit arenas to avoid memory bloat in multi-threaded services.
 * WiFi8 SDK use glibc2.8
 */
       "MALLOC_ARENA_MAX=1",
#endif
...
struct apmx_apgx_nv_default_t {
    char *key;
    char *val;
};
const static struct apmx_apgx_nv_default_t apgx_default[] = {
    {   "enable",       "0"     	},
    {   "ssid",         ""      	},
    {   "hide_ssid",    "0"     	},
    {   "security",     ""      	},
...
static void 
apgx_resotre_defaults(void)
{
	struct apmx_apgx_nv_default_t *p = NULL;
	char prefix[32];
	char tmp[100];
	int i;
	...
	for (i=0; i<MTLAN_MAXINUM; i++) {
		...
		snprintf(prefix, sizeof(prefix), "apg%d_", i);
		for (p=&apgx_default[0]; p->key != NULL; p++) {
			if (nvram_get(strcat_r(prefix, p->key, tmp)))
				nvram_unset(strcat_r(prefix, p->key, tmp));
		}
	}
...
const static struct apmx_apgx_nv_default_t apmx_default[] = {
    {   "enable",       "0"     	},
    {   "ssid",         ""      	},
    {   "hide_ssid",    "0"     	},
    {   "security",     ""      	},
...
apmx_resotre_defaults(void)
{
	...
	for (i=1; i<=MTLAN_MWL_MAXINUM; i++) {
        memset(prefix, 0, sizeof(prefix));
        snprintf(prefix, sizeof(prefix), "apm%d_", i);
		for (p=&apmx_default[0]; p->key != NULL; p++) {
			if (nvram_get(strcat_r(prefix, p->key, tmp)))
				nvram_unset(strcat_r(prefix, p->key, tmp));
		}
	}
...
#endif	// defined(RTCONFIG_MULTILAN_CFG) && defined(RTCONFIG_MULTILAN_MWL)
...
#else
#if defined(RTAX58U_V2) || defined(GTAX6000) || defined(RTAX3000N) || defined(BR63) || defined(RTAX82U_V2) || defined(TUFAX5400_V2) || defined(RTAX88U_PRO) || defined(RTAX5400) || defined(RTBE58U) || defined(TUFBE3600) || defined(RTBE58U_V2) || defined(TUFBE3600_V2) || defined(RTBE55) || defined(RTBE92U) || defined(RTBE95U) || defined(RTBE58U_PRO)
			wan_phy_led_pinmux(0);
#endif
			led_control(LED_WAN_NORMAL, nvram_match("AllLED", "1") ? LED_ON : LED_OFF);
#endif
			return;
#endif
#endif
...
wl_defaults(void)
{
	...
	char word[256], *next;
	int unit, subunit;
	char wlx_vifnames[256], wl_vifnames[256], lan_ifnames[512];
	int subunit_x = 0;
...
*/
		...
		switch(unit) {
			case WL_2G_BAND:
				nvram_set_int(strcat_r(pprefix, "nband_type", tmp), BAND_ATTR_2G);
				break;
#if defined(RTCONFIG_HAS_5G_2)
			case WL_5G_BAND:
				nvram_set_int(strcat_r(pprefix, "nband_type", tmp), BAND_ATTR_5GL);
				break;
			case WL_5G_2_BAND:
				nvram_set_int(strcat_r(pprefix, "nband_type", tmp), BAND_ATTR_5GH);
				break;
#elif defined(RTCONFIG_HAS_5G)
			case WL_5G_BAND:
				nvram_set_int(strcat_r(pprefix, "nband_type", tmp), BAND_ATTR_5G);
				break;
#endif
#if defined(RTCONFIG_HAS_6G_2)
			case WL_6G_BAND:
				nvram_set_int(strcat_r(pprefix, "nband_type", tmp), BAND_ATTR_6GL);
				break;
			case WL_6G_2_BAND:
				nvram_set_int(strcat_r(pprefix, "nband_type", tmp), BAND_ATTR_6GH);
				break;
#elif defined(RTCONFIG_HAS_6G) || defined(RTCONFIG_WIFI6E)
			case WL_6G_BAND:
				nvram_set_int(strcat_r(pprefix, "nband_type", tmp), BAND_ATTR_6G);
				break;
#endif
		}
#endif //defined(RTCONFIG_BCMWL6)
...
/* [MUST]: Why to do this ?  */
			...
			if (subunit==1 && nvram_match("x_Setting", "0")) {
				nvram_set(strcat_r(prefix, "bss_enabled", tmp), "1");
			}
#endif
...
//				nvram_set(strcat_r(prefix, "ifname", tmp), tmp2);
			...
			if(strlen(nvram_safe_get(strcat_r(prefix, "wifi_mode", tmp))) == 0)
					nvram_set(strcat_r(prefix, "wifi_mode", tmp), "ax");
...
#if defined(RTCONFIG_MULTILAN_CFG)
	{
	struct apmx_apgx_nv_default_t *apgx_default_p = NULL;
	for (i=0; i<MTLAN_MAXINUM; i++) {
		memset(tmp2, 0, sizeof(tmp2));
		snprintf(tmp2, sizeof(tmp2), "apg%d_", i);
		for (apgx_default_p = &apgx_default[0]; apgx_default_p->key != NULL; apgx_default_p++) {
			if (!nvram_get(strcat_r(tmp2, apgx_default_p->key, tmp)))
				nvram_set(strcat_r(tmp2, apgx_default_p->key, tmp), apgx_default_p->val);
		}
	}
	}
#endif	// RTCONFIG_MULTILAN_CFG
...
#if defined(RTCONFIG_MULTILAN_CFG) && defined(RTCONFIG_MULTILAN_MWL) 
	{
	struct apmx_apgx_nv_default_t *apmx_default_p = NULL;
    for (i=1; i<=MTLAN_MWL_MAXINUM; i++) {
        memset(tmp2, 0, sizeof(tmp2));
		snprintf(tmp2, sizeof(tmp2), "apm%d_", i);
		for (apmx_default_p = &apmx_default[0]; apmx_default_p->key != NULL; apmx_default_p++) {
			if (!nvram_get(strcat_r(tmp2, apmx_default_p->key, tmp))) 
				nvram_set(strcat_r(tmp2, apmx_default_p->key, tmp), apmx_default_p->val);
		}
	}
	}
...
restore_defaults_wifi(int all)
{
	...
	rev3 = 1;
#endif
	_dprintf("%s, rev3=%d, all=%d\n", __func__, rev3, all);

	unit = 0;
	foreach (word, nvram_safe_get("wl_ifnames"), next) {
		SKIP_ABSENT_BAND_AND_INC_UNIT(unit);
		snprintf(prefix, sizeof(prefix), "wl%d_", unit);
		snprintf(prefix_main, sizeof(prefix_main), "wl%d_", unit);
		max_mssid = num_of_mssid_support(unit);
		...
		for (subunit = 1; subunit < max_mssid+1; subunit++) {
			...
			if (subunit == 1 && nvram_match("x_Setting", "0"))
				strlcpy(ssid, nvram_safe_get(strcat_r(prefix_main, "ssid", tmp)), sizeof(ssid));
			else
#endif
...
misc_defaults(int restore_defaults)
{
	...
	// default for USB state control variables. {
			...
			if(nvram_match("wans_usb_bk", "1") && nvram_match("wans_usb_bk_act", "1"))
			{
				...
				if (strncmp(wans_mt_ioport, "usb", 3) && strncmp(wans_mt_ioport, "U", 1))
				{
					p = strchr(wans_mt_ioport, ' ');
					if (p)
						*p = '\0';
					nvram_set("wans_mt_ioport", wans_mt_ioport);
					nvram_set("mtwan1_mt_group", "1");
					nvram_set("mtwan1_mt_weight", "1");
					nvram_set("mtwan1_order", "1");
					nvram_set_int("mtwan1_enable", 0);
					nvram_set_int("wans_usb_bk_act", 0);
					nvram_set_int("wan_primary", 0);
					nvram_set_int("wan0_primary", 1);
					nvram_set_int("wan51_primary", 0);
				}
#else
...
#endif //defined(RTCONFIG_USB_WAN_BACKUP ) && defined (RTCONFIG_DUALWAN)
...
#define AAE_EANBLE_AICLOUD 4
	...
	if (strlen(nvram_safe_get("log_wlstat_dir")) &&
		...
		char cmd[128];
		safe_do_system("rm -rf %s", nvram_safe_get("log_wlstat_dir"));
	}
#endif
...
restore_defaults(void)
{
...
#if defined(RTCONFIG_MULTILAN_CFG)
		apgx_resotre_defaults();
#endif	// RTCONFIG_MULTILAN_CFG

#if defined(RTCONFIG_MULTILAN_CFG) && defined(RTCONFIG_MULTILAN_MWL)
		apmx_resotre_defaults();
#endif	// defined(RTCONFIG_MULTILAN_CFG) && defined(RTCONFIG_MULTILAN_MWL)
 	}
#if defined(RTCONFIG_BCM_7114) || defined(HND_ROUTER)
	else
	...
	if (restore_defaults) {
...
#ifdef CONFIG_BCMWL5
		if (memsize > 400 * 1024)
			nvram_set_int("log_size", 1024);
		else if (memsize > 200 * 1024)
		...
		if(lan_ipaddr != NULL && strcmp(lan_ipaddr, "NONE") && is_valid_ipv4(lan_ipaddr)){
...
#if defined(RTCONFIG_HND_ROUTER_AX_6756) || defined(RTCONFIG_HND_ROUTER_BE_4916)
static void console_override(void)
{
	...
	if (f_read_string("/proc/environment/noconsole", value, sizeof(value)) > 0) {
		...
		nvram_set_int("noconsole", noconsole);
	}
}
#endif
...
static void shutdn(int rb)
{
	...
	/* record software reboot */
	time_t now;
	time(&now);
	logmessage("REBOOT", "soft-reboot timestamp=%ld", now);
	...
	sync(); sync(); sync();
#ifdef HND_ROUTER
#if defined(BCM6855) || defined(GTAXE16000) || defined(GTBE98) || defined(GTBE98_PRO) || defined(RTBE96U) || defined(GTBE96) || defined(BQ16) || defined(BQ16_PRO) || defined(BN12) || defined(GTBE19000) || defined(BT10) || defined(GTBE19000AI) || defined(EBG15) || defined(EBG19P) || defined(GTBE96_AI) || defined(RTBE88U) || defined(GTBN98_PRO) || defined(GTBN96X) || defined(GTBN98) || defined(GTBN96)
	config_jumbo_frame();
#endif
#if defined(RTBE88U) || defined(RTBE96U) || defined(GTBE96) || defined(GTBE98) || defined(GTBE98_PRO) || defined(GTBE19000) || defined(GTBE19000AI) || defined(GTBE96_AI)
	ed_thresh_clear();
#endif
#if defined(RTCONFIG_HND_ROUTER_AX_6756) || defined(RTCONFIG_HND_ROUTER_BE_4916)
...
static int set_basic_ifname_vars(char *wan_ifaces[MAX_WAN_IFACE_ID], char *lan, char *wl_ifaces[WL_NR_BANDS], char *usb, char *ap_lan, char *dw_wan, char *dw_lan, int force_dwlan)
{
    ...
    defined(RTCONFIG_SWITCH_RTL8370MB_PHY_QCA8033_X2)
			...
			if (wphy) {
#if defined(RTCONFIG_WISP) && !defined(RTCONFIG_HND_ROUTER_BE_4916)
				if (wisp_mode()) {
					add_lan_phy(wphy);
...
void init_ahs_dhcp_params(void)
{
	nvram_set("ahs_dhcp_sent", "0");
	if(nvram_match("ahs_dhcp_log_ext", "A"))
	{
		//Change from A to B
		nvram_set("ahs_dhcp_log_ext", "B");
	}
	else
	{
		//Change from B or NULL to A
		nvram_set("ahs_dhcp_log_ext", "A");
	}
}
#endif /* RTCONFIG_AHS */
...
int init_nvram(void)
{
	...
	nvram_set("fb_bootstate_flag", "init");
#endif
...
//		add_rc_support("pwrctrl");
	...
	case MODEL_EBG15:
                ...
                nvram_set("ethmon_ifnames", "eth1 eth2 eth3 eth4");
#endif
...
*/
	...
	case MODEL_EBG19P:
		...
		nvram_set_int("led_pwr_gpio", 52|GPIO_ACTIVE_LOW);
		nvram_set_int("led_poe_alert_gpio", 55|GPIO_ACTIVE_LOW);
		nvram_set_int("btn_rst_gpio", 56|GPIO_ACTIVE_LOW);
...
*/
		...
		add_rc_support("ofdma");
		add_rc_support("WIFI_LOGO");
                add_rc_support("802_3_at");

		update_ebg19p_defaults();
                break;
#endif
...
//		nvram_set("bcm43684_nic", "1");	// bring up bcm43684 in nic mode
		...
		nvram_set("amas_ethif_type", "4 32"); // 1G, 10G
		nvram_set("sta_ifnames", "wl2 wl1 wl0");
		nvram_set("wired_ifnames", "eth2");

		nvram_set("eth_priority", "0 1 1 1 2 1"); // eth0: 1G(idx:0,prio:1,used:1), eth1: 10G(idx:0,prio:1,used:1)
		...
		nvram_set("amas_lldp_iftypes", "4 32");
#endif
...
//		nvram_set("bcm43684_nic", "1");	// bring up bcm43684 in nic mode
	...
	case MODEL_BN12:
		...
		nvram_set("lan_ifname", "br0");
		if (is_router_mode()) {
			nvram_set("lan_ifnames", "eth1 eth2 eth3 eth4 wl2 wl1 wl0");
			nvram_set("wan_ifname", "eth0");
			nvram_set("wan_ifnames", "eth0");
			reconfig_manual_wan_ifnames();
		} else {
			nvram_set("lan_ifnames", "eth0 eth1 eth2 eth3 eth4 wl2 wl1 wl0");
			nvram_set("wan_ifnames", "");
			nvram_set("wan_ifname", "");
		}
		if(nvram_get_int("sw_mode") == 2){
...
//		nvram_set("bcm43684_nic", "1");	// bring up bcm43684 in nic mode
		...
		nvram_set("eth_priority", "0 1 1 1 2 1"); // eth0: 10G(idx:0,prio:1,used:1) > eth1: 2.5G(idx:1,prio:2,used:1)
		nvram_set("sta_priority", "6 2 3 1 5 1 4 1 2 0 5 1"); // 6G:(index:2, prio:3, used:1), 5G:(index:1, prio:4, used:1), 2G:(index:0, prio:5, used:1)

		/* interface name & type mapping for lldp */
		nvram_set("amas_lldp_ifnames", "eth0 eth1");
		nvram_set("amas_lldp_iftypes", "32 8");
#endif
		...
		nvram_set_int("led_rgb1_blue_gpio", 4|GPIO_ACTIVE_LOW);
		nvram_set_int("led_white_gpio", 21|GPIO_ACTIVE_LOW);
	...
	case MODEL_RTAX55:
		...
		if (!nvram_match("x_Setting", "1") || nvram_get_int("re_mode") == 1) {
			if (!strncmp(nvram_safe_get("territory_code"), "EU", 2) ||
				...
				nvram_set("sb/0/ed_thresh2g", ED_THRESH_DBG);
...
//		nvram_set("bcm43684_nic", "1");	// bring up bcm43684 in nic mode
	...
	case MODEL_RPBE58:
		nvram_set("lan_ifname", "br0");
		nvram_set("wl_def_ap_ifnames", "wl0.1 wl1.1");
		...
		if (is_router_mode()) {
			...
			nvram_set("wan_ifname", "");
#elif defined(RPBE58)
			//if(nvram_match("disable_dhcp_war", "1"))
			//	nvram_set("dhcp_war", "0");
			//else
			//	nvram_set("dhcp_war", "1");
			nvram_set("lan_ifnames", "eth0 wl0 wl1");
			if(ipv6_enabled() && nvram_match("lan_proto", "dhcp")) {
				nvram_set("wan_ifnames", "eth0");
				nvram_set("wan_ifname", "eth0");
			} else {
				nvram_set("wan_ifnames", "");
		...
		if(nvram_match("x_Setting", "0")) {
			...
			if (nvram_match("mld_test", "1")) {
				_dprintf("%s, pre set mlo vars\n", __func__);
				nvram_set("mld_enable", "1");
				nvram_set("mld0_ifnames", "wl0 wl1");
				nvram_set("wl_mlo_config", "1 0 -1 -1");
				nvram_set("mlo_map", "wl1");
				nvram_set("mlo_aap1", "wl0");
				nvram_set("mlo_rp", "1");
			} else if (nvram_match("mld_test", "2")) {
				_dprintf("%s, pre set mlo vars\n", __func__);
				nvram_set("wl0_11be", "0");	
				nvram_set("wl1_11be", "0");	
				nvram_set("wl_11be", "0");	
				nvram_set("wl_mlo_config", "-1 -1 -1 -1");
			}
#endif
#endif
		} else
...
#ifdef RPBE58
		if (mb_mode() && nvram_match("mlo_mb", "0") && !nvram_match("re_mode", "1") && nvram_match("dpsr_mb", "1"))
		{
			_dprintf("chk convert mb to dpsr_mb mode. mld(%d)\n", nvram_get_int("mld_enable"));
			nvram_set("dpsr_mb", "2");
		...
		nvram_set("wl0_11be", "1");
		nvram_set("wl0_bss_mlo_mode", "1");
		nvram_set("wl0_eht_bssehtmode", "1");
		nvram_set("wl1_11be", "1");
		nvram_set("wl1_bss_mlo_mode", "1");
		nvram_set("wl1_eht_bssehtmode", "1");
#else
		if(nvram_match("x_Setting", "0")) {
		...
		//if(!nvram_match("mlo_rp", "1") && (nvram_match("x_Setting", "0") || nvram_match("mld_enable", "0"))) {
		//	nvram_set("wl_mlo_config", "");
		//	nvram_set("mlo_map", "");
		//}
		...
		if (nvram_get_int("no_ed_thresh_unset") == 0) {
			if (is_CN_sku()) {
				...
				if (strlen(cfe_nvram_safe_get_raw("sb/0/ed_thresh5g")))
					...
					nvram_set_int("sb/0/ed_thresh5g", -72);
			} else if (is_EU_sku()) {
		...
		if (ATE_BRCM_FACTORY_MODE())
			...
			if (is_CN_sku() && !nvram_match("location_code", "XX")) {
				nvram_set("no_dy_ed_thresh_ctrl", "-1");
				nvram_set_int("sb/1/ed_thresh2g", -65);	// by rf 0711 results
				nvram_set_int("sb/0/ed_thresh5g", -70);	// by rf 0711 results
			} else {
				nvram_set("no_dy_ed_thresh_ctrl", "-1");
				if (is_EU_sku()) {
...
#else
		if(nvram_match("force_ed_thresh", "1")) {
		...
		if(dpsr_mode() && nvram_get_int("x_Setting")>0) {
			...
			_dprintf("%s: not reset wlx_ssid due not rp_mode or in default\n", __func__);
...
//		nvram_set("bcm43684_nic", "1");	// bring up bcm43684 in nic mode
	...
	case MODEL_GTAX6000:
		...
		if (nvram_get_int("re_mode") == 1) {
			...
			nvram_set("eth_priority", "0 1 1 1 2 1");	// eth0: 2.5G(idx:0,prio:1,used:1) > eth5: 2.5G(idx:1,prio:2,used:1)
			nvram_set("wired_ifnames", "eth1 eth2 eth3 eth4 eth5");
			nvram_set("sta_ifnames", "eth6 eth7");
...
#if defined(GTBE98) || defined(GTBE98_PRO) || defined(GTBE19000) || defined(GTBE19000AI)
	case MODEL_GTBE98:
	case MODEL_GTBE98_PRO:
	case MODEL_GTBE19000:
	case MODEL_GTBE19000AI:
		...
		update_uboot_env();
#endif
#if defined(RTCONFIG_AIRIQ)
		if(!nvram_match("3:airiq_enable", "1"))
			nvram_set("3:airiq_enable", "1");
#endif
		ed_thresh_override();
		nvram_set("dhd_rnr_offload_override", "7");
		nvram_set("lan_ifname", "br0");
		reconfig_manual_wan_ifnames();
#if defined(GTBE19000) || defined(GTBE19000AI)
		nvram_set("wl_ifnames", "wl0 wl1 wl2");
#else
		...
		if(nvram_get_int("re_mode") == 1)
			if(!is_rtl8372_boardid())
				nvram_set("wired_ifnames", "eth2 eth3 eth4 eth5 eth6");
			else
				nvram_set("wired_ifnames", "eth1 eth2 eth3");
		else {
			if(!is_rtl8372_boardid())
		...
		nvram_set_int("led_group2_blue_gpio", 7);	// sw_parallel_led_7
#if defined(GTBE19000AI)
		nvram_set_int("led_group3_red_gpio", 22);	// sw_parallel_led_22
		nvram_set_int("led_group3_green_gpio", 16);	// sw_parallel_led_16
		nvram_set_int("led_group3_blue_gpio", 23);	// sw_parallel_led_23
#else
		nvram_set_int("led_group3_red_gpio", 14);	// sw_parallel_led_14
		nvram_set_int("led_group3_green_gpio", 16);	// sw_parallel_led_16
		nvram_set_int("led_group3_blue_gpio", 15);	// sw_parallel_led_15
		...
		if ((hnd_boardid_cmp("GT-BE98PRO") == 0))
			nvram_set_int("led_afc_gpio", 15|GPIO_ACTIVE_LOW);      // AFC BLUE RED
		else
			nvram_set_int("led_afc_gpio", 48|GPIO_ACTIVE_LOW);      // AFC BLUE RED
#endif
#if defined(GTBE19000AI)
		nvram_set_int("es_rst_gpio", 51|GPIO_ACTIVE_LOW);
		nvram_set_int("es_sop0_gpio", 71);
		nvram_set_int("es_sop1_gpio", 72);
		nvram_set_int("es_sop2_gpio", 73);
		nvram_set_int("pwr_switch_gpio", 78);
		nvram_set_int("btn_wake_gpio", 79);
		nvram_set_int("pwr_usb_gpio", 59);
#endif
		if (usb_usb3 == 1) {
			nvram_set("xhci_ports", "2-1");
...
//#endif
		...
		nvram_set("ai_productid", "SL1680");
		nvram_set("ai_buildno", "102");
		nvram_set("ai_prog_status", "");
		nvram_set("ai_prog_error_code", "");
		nvram_set("ai_prog_reason", "");
		nvram_set("ai_prog_dwl_percent", "");
		nvram_set("ai_prog_dwl_bytes", "");
		nvram_set("ai_prog_total_steps", "");
		nvram_set("ai_prog_current_step", "");
		nvram_set("ai_prog_current_percent", "");
		nvram_set("ai_prog_current_image", "");
		nvram_set("ai_app_prog_status", "");
		nvram_set("ai_app_prog_error_code", "");
		nvram_set("ai_app_prog_reason", "");
		nvram_set("ai_app_prog_dwl_percent", "");
		nvram_set("ai_app_prog_dwl_bytes", "");
		nvram_set("ai_app_prog_total_steps", "");
		nvram_set("ai_app_prog_current_step", "");
		nvram_set("ai_app_prog_current_percent", "");
		nvram_set("ai_app_prog_current_image", "");
		nvram_set("ai_docker_prog_status", "");
		nvram_set("ai_docker_prog_error_code", "");
		nvram_set("ai_docker_prog_reason", "");
		nvram_set("ai_docker_prog_dwl_percent", "");
		nvram_set("ai_docker_prog_dwl_bytes", "");
		nvram_set("ai_docker_prog_total_steps", "");
		nvram_set("ai_docker_prog_current_step", "");
		nvram_set("ai_docker_prog_current_percent", "");
		nvram_set("ai_docker_prog_current_image", "");
		add_rc_support("ai_support");
		add_rc_support("ai_betaupg");
		add_rc_support("ai_reset_beta");
#endif
		break;
#endif

#if defined(GTBE96) || defined(GTBE96_AI)
	case MODEL_GTBE96:
	case MODEL_GTBE96_AI:
#ifdef RTCONFIG_EXTPHY_BCM84880
		get_ext_phy_id();
...
#if defined(GTBN98_PRO) || defined(GTBN98)
	case MODEL_GTBN98_PRO:
	case MODEL_GTBN98:
#if defined(WIFI8_SDK_20251126)
		nvram_set("dol0_rxoffl", "1:4096:2048");
		nvram_set("dol1_rxoffl", "1:4096:2048");
		nvram_set("dol2_rxoffl", "1:4096:2048");
#endif
#ifdef RTCONFIG_EXTPHY_BCM84880
		get_ext_phy_id();
#endif
		...
		nvram_set_int("led_group3_blue_gpio", 15);	// sw_parallel_led_15
		nvram_set_int("led_group4_red_gpio", 18);       // sw_parallel_led_18
		nvram_set_int("led_group4_green_gpio", 20);     // sw_parallel_led_20
		nvram_set_int("led_group4_blue_gpio", 19);      // sw_parallel_led_19
#if defined(RTCONFIG_PRESSURE_SENSOR) || defined(RTCONFIG_PRESSURE_SENSOR_CMP201) || defined(RTCONFIG_BCM_MFG)
		nvram_set_int("led_afc_gpio", 48|GPIO_ACTIVE_LOW);	// AFC BLUE RED - 6G1
		nvram_set_int("led_afc2_gpio", 74|GPIO_ACTIVE_LOW);	// AFC BLUE RED - 6G2
#endif
		if (usb_usb3 == 1) {
			nvram_set("xhci_ports", "2-1");
			nvram_set("ehci_ports", "1-1 1-2");
...
//		add_rc_support("WIFI_LOGO");
	...
	case MODEL_RTBE86U:
		...
		nvram_set("sta_ifnames", "wl0 wl1");
		nvram_set("wired_ifnames", nvram_get_int("lacp_enabled") ? "bond0 eth3 eth4" : "eth2 eth3 eth4");
...
#ifdef RTBE55
		nvram_set("amas_lldp_ifnames", "eth0");
		nvram_set("amas_lldp_iftypes", "8"); // 2.5G
#else
#if defined(RTCONFIG_BCM_EXT_SWITCH_RTK)
		nvram_set("amas_lldp_ifnames", "eth0 eth1");
#else
...
//		add_rc_support("WIFI_LOGO");
	...
	case MODEL_GSBE18000:
		...
		nvram_set("eth_priority", "0 1 1");			// eth0: 2.5G(idx:0,prio:1,used:1)
#ifdef GS7_PRO
		nvram_set("sta_priority", "5 2 2 1 5 1 3 0 2 0 4 1");	// 5G:(index:2, prio:2, used:1), 5G:(index:1, prio:3, used:0), 2G:(index:0, prio:4, used:1)
#else
		nvram_set("sta_priority", "6 2 2 1 5 1 3 1 2 0 4 1");	// 6G:(index:2, prio:2, used:1), 5G:(index:1, prio:3, used:1), 2G:(index:0, prio:4, used:1)
#endif
		...
		int ed_thresh_5g_old = is_EU_sku() ? nvram_get_int("1:eu_edthresh5g") : nvram_get_int("1:ed_thresh5g");
#ifdef GS7_PRO
		int ed_thresh_6g_old = is_EU_sku() ? nvram_get_int("2:eu_edthresh5g") : nvram_get_int("2:ed_thresh5g");
#else
		int ed_thresh_6g_old = is_EU_sku() ? nvram_get_int("2:eu_edthresh6g") : nvram_get_int("2:ed_thresh6g");
#endif
		...
		if (nvram_get_int("no_ed_thresh_unset") == 0) {
			if (is_CN_sku()) {
				...
				if (strlen(cfe_nvram_safe_get_raw("1:ed_thresh5g")))
					...
					nvram_set_int("1:ed_thresh5g", -68);
				if (strlen(cfe_nvram_safe_get_raw("2:ed_thresh5g")))
...
#else
				if (strlen(cfe_nvram_safe_get_raw("0:eu_edthresh2g")))
					nvram_set("0:eu_edthresh2g", cfe_nvram_safe_get_raw("0:eu_edthresh2g"));
				else
					nvram_set_int("0:eu_edthresh2g", -67);
				if (strlen(cfe_nvram_safe_get_raw("1:eu_edthresh5g")))
					nvram_set("1:eu_edthresh5g", cfe_nvram_safe_get_raw("1:eu_edthresh5g"));
				else
					nvram_set_int("1:eu_edthresh5g", -77);
				if (strlen(cfe_nvram_safe_get_raw("2:eu_edthresh6g")))
					nvram_set("2:eu_edthresh6g", cfe_nvram_safe_get_raw("2:eu_edthresh6g"));
				else
					nvram_set_int("2:eu_edthresh6g", -77);
#endif
			} else {
				nvram_unset("0:ed_thresh2g");
				nvram_unset("1:ed_thresh5g");
...
#else
				if (strlen(cfe_nvram_safe_get_raw("2:ed_thresh6g")))
					nvram_set("2:ed_thresh6g", cfe_nvram_safe_get_raw("2:ed_thresh6g"));
				else
					nvram_set_int("2:ed_thresh6g", -75);
#endif
			}
		}
		...
		nvram_set_int("ed_thresh_reload", (no_dy_ed_thresh_ctrl_old != nvram_get_int("no_dy_ed_thresh_ctrl")) ||
			...
			(ed_thresh_5g_old != (is_EU_sku() ? nvram_get_int("1:eu_edthresh5g") : nvram_get_int("1:ed_thresh5g"))) ||
#ifdef GS7_PRO
			(ed_thresh_6g_old != (is_EU_sku() ? nvram_get_int("2:eu_edthresh5g") : nvram_get_int("2:ed_thresh5g"))) );
#else
			(ed_thresh_6g_old != (is_EU_sku() ? nvram_get_int("2:eu_edthresh6g") : nvram_get_int("2:ed_thresh6g"))) );
#endif

		nvram_set_int("led_pwr_gpio", 10|GPIO_ACTIVE_LOW);
		nvram_set_int("led_wps_gpio", 10|GPIO_ACTIVE_LOW);
		nvram_set_int("led_lan_gpio", 14|GPIO_ACTIVE_LOW);
		nvram_set_int("led_wan_normal_gpio", 12|GPIO_ACTIVE_LOW);	// WAN WHITE
		nvram_set_int("led_wan_gpio", 16|GPIO_ACTIVE_LOW);		// WAN RED
		nvram_set_int("led_wifi_gpio", 17|GPIO_ACTIVE_LOW);
		nvram_set_int("btn_wps_gpio", 18|GPIO_ACTIVE_LOW);
		nvram_set_int("btn_rst_gpio", 6|GPIO_ACTIVE_LOW);
		nvram_set_int("pwr_usb_gpio", 67|GPIO_ACTIVE_LOW);
		...
		nvram_set_int("led_group1_blue_gpio", 4);
		nvram_set_int("led_group2_red_gpio", 29);
		nvram_set_int("led_group2_green_gpio", 31);
		nvram_set_int("led_group2_blue_gpio", 30);
		nvram_set_int("led_group3_red_gpio", 1);
		nvram_set_int("led_group3_green_gpio", 3);
...
//		add_rc_support("WIFI_LOGO");
	...
	case MODEL_GS7_PRO_MAX:
		...
		nvram_set("wired_ifnames", "eth1");
		nvram_set("eth_priority", "0 1 1 1 2 1");		// eth0: 10G(idx:0,prio:1,used:1) > vlan4094(eth1): 2.5G(idx:1,prio:2,used:1)
		nvram_set("sta_priority", "5 2 3 1 5 1 4 0 2 0 5 1");	// 5G:(index:2, prio:3, used:1), 5G:(index:1, prio:4, used:0), 2G:(index:0, prio:5, used:1)
...
//		add_rc_support("WIFI_LOGO");
	...
	case MODEL_GT7:
		...
		nvram_set_int("led_rgb1_red_gpio", 19);
		nvram_set_int("led_rgb1_green_gpio", 21);
		nvram_set_int("led_rgb1_blue_gpio", 4);
		nvram_set_int("led_group1_red_gpio", 22);
		nvram_set_int("led_group1_green_gpio", 24);
		nvram_set_int("led_group1_blue_gpio", 23);
		nvram_set_int("led_group2_red_gpio", 29);
		nvram_set_int("led_group2_green_gpio", 31);
		nvram_set_int("led_group2_blue_gpio", 30);
		nvram_set_int("led_group3_red_gpio", 1);
		...
		nvram_set_int("led_pwr_gpio", 13|GPIO_ACTIVE_LOW);
		nvram_set_int("led_wps_gpio", 13|GPIO_ACTIVE_LOW);
		nvram_set_int("led_wan_normal_gpio", 14|GPIO_ACTIVE_LOW);	// WAN WHITE
		nvram_set_int("led_wan_gpio", 11|GPIO_ACTIVE_LOW);		// WAN RED
		nvram_set_int("led_wifi_gpio", 17|GPIO_ACTIVE_LOW);
...
//		add_rc_support("WIFI_LOGO");
	...
	case MODEL_RTBE58_GO:
		...
		if (nvram_match("x_Setting", "0") && nvram_match("sw_mode", "3")) {
			//nvram_set("lan_ifnames", "eth0 eth1 wl0 wl1 wl0.1 wl1.1");
			//nvram_set("mlo_off", "1");
			nvram_set("lan_ifnames", "eth0 wl0 wl1 wl0.1 wl1.1");
			_dprintf("%s, reset lan_ifnames\n", __func__);
		}
		//nvram_set("wl_ifnames", "wl0 wl1");
		//nvram_set("wl0_vifnames", "wl0.1 wl0.2 wl0.3");
		//nvram_set("wl1_vifnames", "wl1.1 wl1.2 wl1.3");

		if (!nvram_match("mld_enable", "1")) {
			_dprintf("%s:: init mlo config\n", __func__);
			nvram_set("wl_mlo_config", "-1 -1 -1 -1");
		}

		// check slide-switch-mode button 
		char switch_conf[8];
		int switch_current_state = button_pressed(BTN_SWITCH);
		const char *btn_switch_mode = (switch_current_state == 0) ? "mode_0" : "mode_1";
		
                strlcpy(switch_conf, nvram_safe_get("hw_switch_mode"), sizeof(switch_conf));
                _dprintf("init: hw_switch_mode conf:%s, btn:%s\n", switch_conf, btn_switch_mode);
                nvram_set("hw_switch_mode", btn_switch_mode);

                if (strncmp(switch_conf, btn_switch_mode, strlen(btn_switch_mode))) {
                        _dprintf("init: hw_switch_mode changes committed\n");
                        nvram_commit();

                        // TBD
                        // slide_mode_switch(mode);
                }
		...
		nvram_set("eth_ifnames", "eth1");
		nvram_set("wired_ifnames", "eth0");
		nvram_set("amas_ethif_type", "8"); // 2.5G, 1G
		nvram_set("sta_ifnames", "wl0 wl1");
		nvram_set("wl_def_ap_ifnames", "wl0.1 wl1.1");

		nvram_set("eth_priority", "0 1 1"); // eth0: 2.5G(idx:0,prio:1,used:1) > vlan4094(eth1): 1G(idx:1,prio:2,used:1)
		nvram_set("sta_priority", "2 0 3 1 5 1 2 1"); // 2.4G:(prio:4, used:1), 5G:(prio:3, used:1)

		/* interface name & type mapping for lldp */
                //nvram_set("amas_lldp_ifnames", nvram_get_int("lacp_enabled") ? "eth1" : "eth1 eth0");
                //nvram_set("amas_lldp_iftypes", nvram_get_int("lacp_enabled") ? "8" : "8 4");  // 2.5G 1G
                nvram_set("amas_lldp_ifnames", "eth1");
                nvram_set("amas_lldp_iftypes", "8");  // 2.5G 1G
#endif
		int no_dy_ed_thresh_ctrl_old = nvram_get_int("no_dy_ed_thresh_ctrl");
		int ed_thresh_2g_old = is_EU_sku() ? nvram_get_int("sb/1/eu_edthresh2g") : nvram_get_int("sb/1/ed_thresh2g");
		int ed_thresh_5g_old = is_EU_sku() ? nvram_get_int("sb/0/eu_edthresh5g") : nvram_get_int("sb/0/ed_thresh5g");
		...
		if (ATE_BRCM_FACTORY_MODE())
			...
			if (is_CN_sku() && !nvram_match("location_code", "XX")) {
				nvram_set("no_dy_ed_thresh_ctrl", "-1");
				nvram_set_int("sb/1/ed_thresh2g", -65);	// lab_srrc results
				nvram_set_int("sb/0/ed_thresh5g", -72);	// lab srrc results
			} else {
				nvram_set("no_dy_ed_thresh_ctrl", "-1");
				if (is_EU_sku()) {
					nvram_set("sb/1/eu_edthresh2g", "-65");
		...
		nvram_set_int("led_pwr_gpio", 3);
                ...
                nvram_set_int("led_green_gpio", 23|GPIO_ACTIVE_LOW);
                nvram_set_int("led_blue_gpio", 30|GPIO_ACTIVE_LOW);
                nvram_set_int("led_white_gpio", 3);
		nvram_set_int("btn_rst_gpio", 39|GPIO_ACTIVE_LOW);
		nvram_set_int("btn_switch_gpio", 12|GPIO_ACTIVE_LOW);
...
//#if 0
	...
	if (nvram_get_int("success_start_service") == 0)
		nvram_set("switch_wantag_ifname", wan_if_eth());
	mtwan_init_nvram();
#endif
			...
			if (!repeater_mode() && !mediabridge_mode() && get_ap_wifi_rl_from_nvram(ap_wifi_rl, MAX_AP_RULE_LIST, &total) && total > 0) {
				...
				for (j=0; j<total; j++) {
					...
					foreach_44(word, ap_wifi_rl[j].wlif_set, next) {
						...
						trim_space(word);
						snprintf(tmp, sizeof(tmp), "%s_rrm", word);
...
// #endif
	...
	add_led_ctrl_capability(LED_ON_OFF);
#endif
	add_led_ctrl_capability(CENTRAL_LED_ON_OFF);
#endif
#if defined(RTCONFIG_SINGLE_LED_NIGHT_MODE)
	add_led_ctrl_capability(LED_NIGHT_MODE);
#endif
#if defined(RTCONFIG_AURALED_NIGHT)
	add_led_ctrl_capability(AURALED_NIGHT_MODE);
#endif
#endif
...
#ifdef RTCONFIG_FRS_LIVE_UPDATE
	/* reset the counter "afc_req_cnt" to "0" */
	nvram_set("afc_req_cnt", "0");
#endif /* RTCONFIG_FRS_LIVE_UPDATE */
...
#ifdef RTCONFIG_RE_MLO_VSIE_ACTIVE
	add_rc_support("re_mlo_vsie");
#endif
...
//
	...
	/* set wifi generation in misc info */
	wifi_gen_val = get_wifi_gen();
	if (wifi_gen_val != 0) {
		snprintf(wifi_gen, sizeof(wifi_gen), "%u", wifi_gen_val);
		amas_set_misc_info(MISC_INFO_WIFI_GEN, wifi_gen);
	}
#endif /* AMAS */
#endif /* CFGSYNC */
#ifdef RTCONFIG_AMAS
...
#if defined(RTCONFIG_BCM_AFC)
	nvram_unset("afc_cold_reboot");
#endif
...
//		nvram_set("webs_update_enable", "0");
...
#ifdef RTCONFIG_AHS
	init_ahs_bhc_params();
	init_ahs_dhcp_params();
#endif /* RTCONFIG_AHS */
#endif /* RTCONFIG_FRS_LIVE_UPDATE */

#if defined(RTCONFIG_NVSW_IN_JFFS) && (defined(PRTAX57_GO) || defined(RTBE58_GO))
	if (nvram_get("nvsw_ov_onoff")) {
		int val = nvram_get_int("nvsw_ov_onoff");
		_dprintf("NVSW: btnsw_onoff:%d -> %d\n", nvram_get_int("btnsw_onoff"), val);
		nvram_set_int("btnsw_onoff", val);
	}
#endif
	...
	if (!is_valid_ipv4(nvram_safe_get("lan_ipaddr"))) {
...
void BT_chip_reset(int stage)
{
	...
	bt_reset = 34;
#elif defined(RTAX95Q) || defined(XT8PRO) || defined(BT12) || defined(BT10) || defined(BQ16) || defined(BQ16_PRO) || defined(BN12) || defined(BM68) || defined(XT8_V2) || defined(RTAXE95Q) || defined(ET8PRO) || defined(ET8_V2) || defined(RTAX56_XD4) || defined(XD4PRO) || defined(RTAX82_XD6) || defined(XD6_V2) || defined(XC5)
        bt_reset = 29;
	...
	bt_reset = 3;
#elif defined(BC109) || defined(EBG19P) || defined(EBG15)
	bt_reset = 25;
#else
...
static void start_hw_wdt(void)
{
	...
	if (wdt) {
		...
		safe_do_system("wdtctl -d -t %d start", wdt);
	}
}
#endif
...
#endif
#ifdef EBG19P
                if (get_model() == MODEL_EBG19P) {
                        setAllGreenLedOn();
                } else
#endif
#if defined(RPBE58) || defined(RTBE58_GO)
		rc_bcm_cled_ctrl(BCM_CLED_YELLOW, BCM_CLED_STEADY_NOBLINK);
#else
		setAllLedOn();
#endif
#else
...
#ifdef EBG19P
                } else if (get_model() == MODEL_EBG19P) {
                        setAllGreenLedOn();
#endif
		} else
			setAllLedOn();
...
#elif defined(RPBE58)
        	eval("sw", "0xff803050", "0");
		eval("sw", "0xff803020", "0x3e002");	// W, pulse
		eval("sw", "0xff803070", "0");
#endif
		break;
	}
...
static void sysinit(void)
{
    ...
    int k;
#if defined(RTCONFIG_VIF_ONBOARDING) && defined(RTCONFIG_AMAS_5G_ONBOARDING)
	char wl_obvif_set[] = "wlXXXX_obvif_set";
#endif
#endif
#if defined(RTCONFIG_NVSW_IN_JFFS)
	int do_restore_default = 0;
#endif
	...
	_dprintf("\nstart_hw_wdt() before bcm_boot_launcher start...\n");
	start_hw_wdt();
	sleep(1);
...
#if defined(RTCONFIG_NVSW_IN_JFFS) && defined(RTCONFIG_RALINK)
	if (RESTORE_DEFAULTS()) {
		nvram_set("ubifs_on", "1");
		nvram_set("ubifs_clean_fs", "1");
		do_restore_default = 1;
	}
	system("mkdir -p /tmp/var/lock"); // workaround
	set_action(ACT_IDLE); // workaround
	start_jffs2();
	nvram_unset("ubifs_clean_fs");
	if (nvsw_switching(&i, do_restore_default)) { // nvram had switched
		// reset, overwrite some nvram here if needed
		_dprintf("[[[NVRAM SWITCH from %d to %d]]]\n", (i >> 8) & 0xff, i & 0xff);
	}
	unlink(ACTION_LOCK); // workaround
#endif
...
def_boot_reinit:
	...
	if(!nvram_match("restore_defaults", "1")) {
		if(nvram_match("knv_verb", "1"))
			diff_knv(NULL, NULL);
		chk_nv_sanity(re_mode(), 1);
	}
#endif
	...
	if (nvram_get_int("min_free_kbytes_manual"))
		...
		f_write_string("/proc/sys/vm/min_free_kbytes", "4096", 0, 0);
	else if (model==MODEL_RTAX56_XD4 || model==MODEL_XD4PRO || model==MODEL_CTAX56_XD4 || model==MODEL_RPAX56 || model==MODEL_RPAX58 || model==MODEL_XC5 || model == MODEL_EBA63)
		f_write_string("/proc/sys/vm/min_free_kbytes", "40960", 0, 0);
        else if (model== MODEL_EBG15|| model == MODEL_EBG19P || model==MODEL_RPBE58)
                f_write_string("/proc/sys/vm/min_free_kbytes", "10240", 0, 0);
	else if (model==MODEL_RTAX95Q || model==MODEL_XT8PRO || model==MODEL_BT12 || model==MODEL_BT10 || model==MODEL_BQ16 || model==MODEL_BQ16_PRO || model==MODEL_BN12 || model==MODEL_BM68 || model==MODEL_XT8_V2 || model==MODEL_RTAXE95Q || model==MODEL_ET8PRO || model==MODEL_ET8_V2 || model==MODEL_RTAX56U)
...
#if defined(RTCONFIG_VIF_ONBOARDING) && defined(RTCONFIG_AMAS_5G_ONBOARDING)
		snprintf(wl_obvif_set, sizeof(wl_obvif_set), "wl%d_obvif_set", k);
		nvram_unset(wl_obvif_set);
#endif
	}
#endif
	...
	if(RESTORE_DEFAULTS()) {
		...
		nvram_set("ext4fs_on", "1");
		nvram_set("ext4fs_clean_fs", "1");
#endif
	}
#ifdef RPAX56
	if(!def_boot)
#endif
	start_jffs2();
#if defined(RTCONFIG_NVSW_IN_JFFS) && defined(RTCONFIG_HND_ROUTER_BE_4916)
	if (RESTORE_DEFAULTS())
		do_restore_default = 1;
	nvram_unset("ubifs_clean_fs");
	if (nvsw_switching(&i, do_restore_default)) { // nvram had switched
...
int init_main(int argc, char *argv[])
{
...
#if defined(RTCONFIG_HND_ROUTER_BE_4916)
#if defined(RTCONFIG_FLASH_TYPE_EMMC)
	system("mount -t ext4 /dev/data /data; sleep 3; insmod bcm_knvram; sleep 3");
#else
...
#if defined(RTCONFIG_PRESSURE_SENSOR) || defined(RTCONFIG_PRESSURE_SENSOR_CMP201)
	{
		...
		char *argv[]={"/sbin/cmp-pressure", NULL};
#else
...
//		TRACE_PT("main loop signal/state=%d\n", state);
		...
		eval("/usr/sbin/modem_status.sh", "bytes+");
...
#endif
			stop_vlan();
			//stop_logger(); // for record more reboot info
			...
			if ((state == SIGTERM /* REBOOT */) ||
				...
				start_lan_port(5);
#endif
#if defined(RTCONFIG_ENERGY_SAVE) && defined(RTCONFIG_CC3220)
				esr_set_sys(ES_STS_OFF, "INIT");
#endif
...
logmessage("ATE", "boot/continue fail= %d/%d\n", nvram_get_int("Ate_boot_fail"),nvram_get_int("Ate_continue_fail"));
			...
			start_services();
#if defined(RTCONFIG_AI_SERVICE)
			renew_ai_key(ROUTER_PRIVATE_KEY, ROUTER_PUBLIC_KEY, ROUTER_PUBLIC_KEY);
			if(!strlen(nvram_safe_get("ai_hostname")))
			{
				ai_init_default_hostname();
			}
#endif
#if defined(RTBE88U) || defined(BQ16_PRO) || defined(BQ16) || defined(BN12)
...
#if !defined(XT8PRO) && !defined(BT12) && !defined(BT10) && !defined(BQ16) && !defined(BQ16_PRO) && !defined(BN12) && !defined(BM68) && !defined(XT8_V2) && !defined(ET8PRO) && !defined(ET8_V2)
#ifdef RTCONFIG_HND_ROUTER_AX
...
#if defined(GTBE19000AI) || (GTBE96_AI)
						snprintf(aiboard_ip, sizeof(aiboard_ip), "%s", nvram_safe_get("Ate_aiboard_ip"));
						if (aiboard_ip[0] == '\0') snprintf(aiboard_ip, sizeof(aiboard_ip), "%s", AIBOARD_CONTROL_IP);
						logmessage("ATE", "aiboard ip = [%s]\n", aiboard_ip);
						if ((fp = fopen("/etc/hosts", "a")) != NULL) {
							fprintf(fp, "%s sl1680\n", aiboard_ip);
							fclose(fp);
						}
						else perror("/etc/hosts");
						while(aiboard_retry < 10){
							aiboard_alive = system("ping -c 1 sl1680");
							if(aiboard_alive == 0) break;
							aiboard_retry++;
						}
						logmessage("ATE", "AIBoard retry: %d | AIBoard alive: %d\n", aiboard_retry, aiboard_alive);
						_dprintf("ATE AIBoard retry: %d | AIBoard alive: %d\n", aiboard_retry, aiboard_alive);
#endif
						ate_stress_pkteng();
					}
					else {
...
#if !defined(RTCONFIG_BCM_MFG)
#if defined(RTBE58_GO)
			if (!nvram_match("pd_low", "1") && !nvram_match("usb_enable", "0"))
#endif
			{
...
#if defined(RTCONFIG_AMAS) && defined(RTCONFIG_VIF_ONBOARDING)
void init_onboarding_vif_flag(void)
{
#ifdef RTCONFIG_AMAS_5G_ONBOARDING
	char wl_prefix[sizeof("wlXXXXX_")], tmp[64];
	int i = 0;

	for (i = 0; i < num_of_wl_if(); i++) {
		snprintf(wl_prefix, sizeof(wl_prefix), "wl%d_", i);
		nvram_unset(strcat_r(wl_prefix, "obvif_set", tmp));
	}

	nvram_set("obvif_init", "1");
#else
...
{
	char prefix[sizeof("wlXXXXX_")], prefix_obvif[sizeof("wlXXXXX_")], tmp[64], obvif_ssid[33], obvif_psk[33];
	char wl_ifnames[64], word[16], *next;
	int unit = 0, obvif_subunit = 0;
#ifdef RTCONFIG_AMAS_5G_ONBOARDING
	...
	if (!nvram_get_int("x_Setting") || (nvram_get_int("re_mode") == 1 && nvram_get_int("cfg_first_sync") == 1)) {
		_dprintf("%s(%d): don't set obvif security\n", __FUNCTION__, __LINE__);
		return;
	}
#endif
	...
	foreach (word, wl_ifnames, next) {
		...
		if (nvram_match(strcat_r(prefix, "auth_mode_x", tmp), "sae") || nvram_match(strcat_r(prefix, "closed", tmp), "1")) {
			memset(obvif_ssid, 0, sizeof(obvif_ssid));
			memset(obvif_psk, 0, sizeof(obvif_psk));
			if (
#ifdef RTCONFIG_AMAS_5G_ONBOARDING
				!nvram_get_int(strcat_r(prefix, "obvif_set", tmp))
#else
				!nvram_get_int("obvif_set")
#endif
...
#ifdef RTCONFIG_AMAS_5G_ONBOARDING
				nvram_set(strcat_r(wl_prefix, "obvif_set", tmp), "1");
#else
...
{
	char prefix[sizeof("wlXXXXX_")], prefix_obvif[sizeof("wlXXXXX_")], tmp[64];
	int ret = 0, obvif_subunit = 0;
#ifdef RTCONFIG_AMAS_5G_ONBOARDING
	char wl_prefix[sizeof("wlXXXXX_")];
...
#ifndef RTCONFIG_AMAS_5G_ONBOARDING
	if (unit != WL_2G_BAND)
		return ret;
#endif

	if (obvif_subunit == 0 || subunit != obvif_subunit)
		return ret;
...
#ifdef RTCONFIG_AMAS_5G_ONBOARDING
void set_onboarding_vif_bss_enabled_by_unit(int unit)
{
	int subunit = 0, max_mssid = num_of_mssid_support(unit);
...
void set_onboarding_vif_ifnames()
{
	char lan_ifnames[255], wl_prefix[sizeof("wlXXXXX_")], wlxy_prefix[sizeof("wlX.YYYYY_")], tmp[64], obvifname[16];
	int i = 0, obvif_subunit = 0, update = 0;

	strncpy(lan_ifnames, nvram_safe_get("lan_ifnames"), sizeof(lan_ifnames));

	if (strlen(lan_ifnames)) {
		/* remove all obvif in lan_ifnames */
		for (i = 0; i < num_of_wl_if(); i++) {
			snprintf(wl_prefix, sizeof(wl_prefix), "wl%d_", i);
			...
			if (obvif_subunit > 0) {
				snprintf(wlxy_prefix, sizeof(wlxy_prefix), "wl%d.%d_", i, obvif_subunit);
				strlcpy(obvifname, nvram_safe_get(strcat_r(wlxy_prefix, "ifname", tmp)), sizeof(obvifname));
				if (strlen(obvifname) && remove_from_list(obvifname, lan_ifnames, sizeof(lan_ifnames)) == 0)
					update = 1;
			}
		}

		/* add obvif (if need) in lan_ifnames */
		for (i = 0; i < num_of_wl_if(); i++) {
			snprintf(wl_prefix, sizeof(wl_prefix), "wl%d_", i);
			...
			if (obvif_subunit > 0) {
				snprintf(wlxy_prefix, sizeof(wlxy_prefix), "wl%d.%d_", i, obvif_subunit);
				if (nvram_get_int(strcat_r(wlxy_prefix, "bss_enabled", tmp))) {
					strlcpy(obvifname, nvram_safe_get(strcat_r(wlxy_prefix, "ifname", tmp)), sizeof(obvifname));
					if (strlen(obvifname) && add_to_list(obvifname, lan_ifnames, sizeof(lan_ifnames)) == 0)
						update = 1;
				}
			}
		}
...
void init_obvif_subunit()
{
	int unit = 0, unit_total = 0;
	int nband_type = 0, update = 0;
	...
	if (nvram_get_int("obvif_init")) {
		unit_total = num_of_wl_if();
		for (unit = 0; unit < unit_total; unit++) {
			obvif_subunit = 0;
			obvif_subunit_tmp = -1;
			obvif_subunit_avl = -1;
			snprintf(wl_prefix, sizeof(wl_prefix), "wl%d_", unit);
			...
			if (nband_type == 0 || nband_type == 1 || nband_type == 3)
			{
				if (IS_CAP())
					obvif_subunit_tmp = nvram_get_int(strcat_r(wl_prefix, "obvif_cap_subunit", tmp));
				else if (IS_RE())
					obvif_subunit_tmp = nvram_get_int(strcat_r(wl_prefix, "obvif_re_subunit", tmp));

				if (obvif_subunit_tmp == 0) {
					if (nband_type == 0) {	/* for 2g */
						if (IS_CAP())
							obvif_subunit = nvram_get_int("obvif_cap_subunit");
						else if (IS_RE())
							obvif_subunit = nvram_get_int("obvif_re_subunit");
					}
					else if (nband_type == 1 || nband_type == 3)	/* for 5g/5g high */
					{
						obvif_subunit_avl = get_avl_obvif_subunit_by_unit(unit);
						if (obvif_subunit_avl > 0)
							obvif_subunit = obvif_subunit_avl;
						else
						{
							_dprintf("%s(%d): no available ob vif for wl%d\n", __FUNCTION__, __LINE__, unit);
							continue;
						}
					}
					...
					if (obvif_subunit > 0) {
						if (IS_CAP()) {
							if (nvram_get_int(strcat_r(wl_prefix, "obvif_cap_subunit", tmp)) != obvif_subunit) {
								_dprintf("%s(%d): update wl%d_obvif_cap_subunit [%d]\n", __FUNCTION__, __LINE__, unit, obvif_subunit);
								nvram_set_int(strcat_r(wl_prefix, "obvif_cap_subunit", tmp), obvif_subunit);
								update = 1;
							}
						}
						else if (IS_RE())
						{
							if (nvram_get_int(strcat_r(wl_prefix, "obvif_re_subunit", tmp)) != obvif_subunit) {
								_dprintf("%s(%d): update wl%d_obvif_re_subunit [%d]\n", __FUNCTION__, __LINE__, unit, obvif_subunit);
								nvram_set_int(strcat_r(wl_prefix, "obvif_re_subunit", tmp), obvif_subunit);
								update = 1;
							}
						}
					}
		...
		nvram_unset("obvif_init");
	}
}
#endif	/* RTCONFIG_AMAS_5G_ONBOARDING */
#endif
...
void reconfig_manual_wan_ifnames(void) {
...
#if defined(RTBE58U) || defined(TUFBE3600) || defined(RTBE58U_V2) || defined(TUFBE3600_V2) || defined(GTBE19000) || defined(RTBE92U) || defined(RTBE95U) || defined(RTBE82U) || defined(TUFBE82) || defined(RTBE58U_PRO) || defined(GT7) || defined(GS7_PRO_MAX)
...
#elif defined(RTBE86U) || defined(GTBN98_PRO) || defined(GTBN98) || defined(GTBN96X)
	snprintf(wan_ifname, sizeof(wan_ifname), "%s", (is_router_mode() && nvram_get_int("wans_extwan")) ? "eth1" : nvram_safe_get("wan_ifname_x"));
#else
	...
	switch (model) {
		...
		case MODEL_GTBN96:
			...
			if (is_rtl8372_boardid())
				...
				snprintf(all_ifnames, sizeof(all_ifnames), "eth0 eth1 eth2 eth3 eth4 eth5 eth6 wl0 wl1 wl2");
#elif defined(GTBE19000AI) || defined(GTBE96_AI)
			if (is_rtl8372_boardid())
				snprintf(all_ifnames, sizeof(all_ifnames), "eth0 eth1 eth2 eth3 eth.ai wl0 wl1 wl2");
			else
				snprintf(all_ifnames, sizeof(all_ifnames), "eth0 eth1 eth2 eth3 eth4 eth5 eth6 eth.ai wl0 wl1 wl2");
#elif defined(GTBN98_PRO) || defined(GTBN98)
				snprintf(all_ifnames, sizeof(all_ifnames), "eth0 eth1 eth2 eth3 eth4 eth5 eth6 wl0 wl1 wl2 wl3");
#elif defined(GTBN96X) || defined(GTBN96)
				snprintf(all_ifnames, sizeof(all_ifnames), "eth0 eth1 eth2 eth3 eth4 eth5 eth6 wl0 wl1 wl2");
#else
			if (is_rtl8372_boardid())
			...
			if (!nvram_get_int("x_Setting") || 
			    (is_router_mode() && !(nvram_get_int("wans_extwan") && (get_wans_dualwan() & WANSCAP_LAN) && nvram_match("wans_lanport", "1"))) ||
			    re_mode()) {
				if (!(is_router_mode() && !nvram_get_int("wans_extwan") && (get_wans_dualwan() & WANSCAP_LAN))) {
					...
					add_to_list("eth1", avbl_wan_ifnames, sizeof(avbl_wan_ifnames));
#else
					if (is_rtl8372_boardid()) {
...
#if defined(GTBE19000AI) || defined(GTBE96_AI) || defined(GTBN98_PRO) || defined(GTBN96X) || defined(GTBN98) || defined(GTBN96)
							} else {
								snprintf(wan1_ifname, sizeof(wan1_ifname), "eth%d", nvram_get_int("wans_lanport"));
								add_wan_phy(wan1_ifname);
								remove_from_list(wan1_ifname, all_ifnames, sizeof(all_ifnames));
...
#if defined(GTBE96) || defined(GTBE19000) || defined(GTBE19000AI) || defined(GTBE96_AI) || defined(GTBN96X)
							add_wan_phy("wl0");
				...
				if (find_in_list(avbl_wan_ifnames, wan_ifname)) {
					...
					if (nvram_get_int("wans_extwan"))
						...
						if (is_rtl8372_boardid())
							...
							nvram_set("lan_ifnames", "eth0 eth2 eth3 eth4 eth5 eth6 wl0 wl1 wl2");
#elif defined(GTBE19000AI) || defined(GTBE96_AI)
						if (is_rtl8372_boardid())
							nvram_set("lan_ifnames", "eth0 eth1 eth2 eth3 eth.ai wl0 wl1 wl2");
						else
							nvram_set("lan_ifnames", "eth0 eth2 eth3 eth4 eth5 eth6 eth.ai wl0 wl1 wl2");
#elif defined(GTBN98_PRO) || defined(GTBN98)
							nvram_set("lan_ifnames", "eth0 eth2 eth3 eth4 eth5 eth6 wl0 wl1 wl2 wl3");
#elif defined(GTBN96X) || defined(GTBN96)
							nvram_set("lan_ifnames", "eth0 eth2 eth3 eth4 eth5 eth6 wl0 wl1 wl2");
#else
						if (is_rtl8372_boardid())
						...
						if (is_rtl8372_boardid())
							...
							nvram_set("lan_ifnames", "eth1 eth2 eth3 eth4 eth5 eth6 wl0 wl1 wl2");
#elif defined(GTBE19000AI) || defined(GTBE96_AI)
						if (is_rtl8372_boardid())
							nvram_set("lan_ifnames", "vlan4094 eth1 eth2 eth3 eth.ai wl0 wl1 wl2");
						else
							nvram_set("lan_ifnames", "eth1 eth2 eth3 eth4 eth5 eth6 eth.ai wl0 wl1 wl2");
#elif defined(GTBN98_PRO) || defined(GTBN98)
							nvram_set("lan_ifnames", "eth1 eth2 eth3 eth4 eth5 eth6 wl0 wl1 wl2 wl3");
#elif defined(GTBN96X) || defined(GTBN96)
							nvram_set("lan_ifnames", "eth1 eth2 eth3 eth4 eth5 eth6 wl0 wl1 wl2");
#else
						if (is_rtl8372_boardid())
		...
		case MODEL_RTBE58U_V2:
			...
			snprintf(avbl_wan_ifnames, sizeof(avbl_wan_ifnames), "eth0");
#if defined(RTCONFIG_BCM_EXT_SWITCH_RTK)
			add_to_list("eth2", all_ifnames, sizeof(all_ifnames));
			add_to_list("eth3", all_ifnames, sizeof(all_ifnames));
			add_to_list("eth4", all_ifnames, sizeof(all_ifnames));
#else
#ifndef RTBE55
			if (vlan4094_enabled()) {
				add_to_list("vlan4094", all_ifnames, sizeof(all_ifnames));
...
#if defined(RTCONFIG_AUTO_WANPORT) && !defined(RTCONFIG_BCM_MFG) && !defined(RTBE55)
#if defined(RTCONFIG_BCM_EXT_SWITCH_RTK)
				nvram_set("autowan_ifnames", "eth0 eth1");
#else
				nvram_set("autowan_ifnames", "eth0 vlan4094");
		...
		case MODEL_RTBE82M:
			snprintf(all_ifnames, sizeof(all_ifnames), "eth0 eth1 wl0 wl1");
			snprintf(avbl_wan_ifnames, sizeof(avbl_wan_ifnames), "eth0");
#if defined(RTCONFIG_BCM_EXT_SWITCH_MXL)
                        add_to_list("eth2", all_ifnames, sizeof(all_ifnames));
                        add_to_list("eth3", all_ifnames, sizeof(all_ifnames));
                        add_to_list("eth4", all_ifnames, sizeof(all_ifnames));
#else
#if defined(RTBE82U) || defined(TUFBE82)
			if (vlan4094_enabled()) {
				add_to_list("vlan4094", all_ifnames, sizeof(all_ifnames));
				add_to_list("vlan4094", avbl_wan_ifnames, sizeof(avbl_wan_ifnames));
			}
...
#if defined(RTCONFIG_AUTO_WANPORT) && !defined(RTCONFIG_BCM_MFG) && !defined(RTBE82M)
				nvram_set("autowan_ifnames", "eth0 vlan4094");
#endif
#else //RTCONFIG_DUALWAN
				if (find_in_list(avbl_wan_ifnames, wan_ifname)) {
				...
				if (nvram_get("wans_dualwan")) {
					...
					for(unit = WAN_UNIT_FIRST; unit < WAN_UNIT_MAX; ++unit) {
						if (get_dualwan_by_unit(unit) == WANS_DUALWAN_IF_WAN) {
							...
							/* set dualwan of rtkswitch(LAN) config on config_switch() */
							if ((nvram_get_int("wans_extwan") == 1) && nvram_match("wans_lanport", "5")) {
								add_wan_phy("eth0");
								remove_from_list("eth0", all_ifnames, sizeof(all_ifnames));
							} else
								add_wan_phy("vlan2");
		...
		case MODEL_RTBE58_GO:
			snprintf(all_ifnames, sizeof(all_ifnames), "eth0 eth1 wl0 wl1");
			snprintf(avbl_wan_ifnames, sizeof(avbl_wan_ifnames), "eth1 eth0");

			if (is_router_mode()) {
#if defined (RTCONFIG_WISP) && defined(RTCONFIG_HND_ROUTER_BE_4916)
				if (wisp_mode())
					nvram_set("wan_ifname", get_wisp_ifname());
				else
#endif /* RTCONFIG_WISP */
				if (find_in_list(avbl_wan_ifnames, wan_ifname))
					nvram_set("wan_ifname", wan_ifname);
	...
	case MODEL_RTBE86U:
		...
		case MODEL_BN12:
			snprintf(all_ifnames, sizeof(all_ifnames), "eth0 eth1 eth2 eth3 eth4 wl2 wl1 wl0");
			snprintf(avbl_wan_ifnames, sizeof(avbl_wan_ifnames), "eth0 eth1");
